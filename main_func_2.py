...
def mainsimu_2():
    ...
    save_path = pathlib.Path(r'D:\control2020\RMP_exp\Exp_data_home\2021_02_02')  # 家
    #save_path = pathlib.Path(r'C:\Users\Elis\Documents\制御2020\リーマン制御2020\Exp_Data_scgool\2021_02_01')  # 大学
    ...
    ### シミュレーション環境 ###
    posi_o_fix, posi_o_move_vs, posi_o_move_inits, posi_o_move_init, waypoint, q_init = env7()  # 環境読み込み
    ...
    slip_angle = np.array([[0]], np.float32)  # 横滑り角
    ...
    state_history = np.concatenate([np.array([[0]], dtype = np.float32), 
                                    ...
                                    dvdt_command.T, 
                                    xi_global_all.T.reshape([1, 24]), 
                                    slip_angle], axis = 1)
    RMPc_history = np.zeros((1, 74), dtype = np.float32)  # fとAを格納
    obs_history = posi_o_move_init.reshape(1, posi_o_move_init.shape[0] * 2)
    ...
    for t in np.arange(time_interval, time_span + time_interval, time_interval, dtype = np.float32):
        ...
        ## 障害物を動かす
        #om1_center = np.mod(om1_init + om1_v * t, 10)
        #om1 = make_squre_obs(H = 1, W = 0.5, theta = pi/3, position_center = om1_center)
        #om2_center = np.mod(om2_init + om2_v * t, 10)
        #om2 = make_squre_obs(H = 0.5, W = 0.5, theta = 0, position_center = om2_center)
        #posi_o_move = np.concatenate([om1, om2], axis = 0)

        om1_center = posi_o_move_inits[0] + posi_o_move_vs[0] * 50
        om1 = make_squre_obs(H = 1.5, W = 1.5, theta = 0, position_center = om1_center, dob = 0.2)
        posi_o_move = om1
        ...
        if np.linalg.norm(posi_g.T - xi_global_all[:, 0:1]) < error_r:  # ウェイポイント到達判定
            ...
            if waypoint_count > waypoint.shape[0]-1:
                print("目標到達！")
                succes, divergence, stack, collision, timeover = True, False, False, False, False
                break
            else:
            ...
            print("制御指令が発散")
            succes, divergence, stack, collision, timeover = False, True, False, False, False
            break

        #elif (np.linalg.norm(q[0:2, :] - state_history[stationary_judg_index:stationary_judg_index + 1, 1:3].T) < 0.001) and (t > time_freeze):  # 停留判定
        #    print("停留点")
        #    succes, divergence, stack, collision, timeover = False, False, True, False, False
        #    break

        elif np.min(np.linalg.norm(q[0:2, :] - posi_o.T, axis = 0)) < L / 2:  # 衝突判定
            print("障害物に衝突")
            succes, divergence, stack, collision, timeover = False, False, False, True, False
            break
            ...
            #q = q + dqdt * time_interval  # 状態遷移を実行
            #slip_angle = np.array([[0]], np.float32)  # これが仮定


            # 状態遷移（ルンゲクッタ）
            if abs(v_command[0, 0]) < 1e-3:  # 並進速度が微小
                print("1")
                dqdt = transform_velocity(L, q, v_command, beta)  #車両中心速度を計算
                q = q + dqdt * time_interval  # 状態遷移を実行
                slip_angle = np.array([[0]], np.float32)  # これが仮定
                ...
                print("2")
                q, dqdt, slip_angle = carsimu_2(q, dqdt, v_command, slip_angle)
    ...
    tend = t  # シミュレーション終了時刻
    if tend > time_span - time_interval:
        succes, divergence, stack, collision, timeover = False, False, False, False, True
    ...
    #axes[2,3].plot(state_history[:, 0].T, np.mod(state_history[:, 36].T, 2*pi), marker="")
    axes[2,3].plot(state_history[:, 0].T, state_history[:, 36].T, marker = "")
    axes[2,3].set_title('slip_angle')
    axes[2,3].set_xlabel('t')
    axes[2,3].set_ylabel('slip_angle')
    axes[2,3].set_xlim(0, tend)
    #axes[2,3].set_ylim(-alpha_g[0], alpha_g[0])
    axes[2,3].grid(True)
