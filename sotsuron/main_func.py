...
・目標到達＆障害物回避のために粗いウェイポイントを使用する
・動的障害物に対してはウェイポイントを無視するようにする
"""
...
def mainsimu():
    ...
    #save_path = pathlib.Path(r'D:\control2020\RMP_exp\Exp_data_home\2021_02_14')  # 家
    ...
    for t in np.arange(time_interval, time_span + time_interval, time_interval, dtype = np.float32):
        ...
        if len(posi_o_move_init) == 0:
            ...
            #om1_center = posi_o_move_inits[0] + posi_o_move_vs[0] * t + np.array([[-3, 0]]).T
            om1_center = posi_o_move_inits[0] + posi_o_move_vs[0] * t
        ...
        #    break
            ...
            for i in np.arange(0, 12, 1):
                ...
                #fci, Aci = compute_cp_RMP2_nonvec(xi_global_all[:, i:i+1], dxidt_global, posi_g, posi_o, jend, i)
    ...
    if len(posi_o_fix) == 0:
        ...
        ax.scatter(posi_o_fix[:, 0].T, posi_o_fix[:, 1].T, 
                    s = 80, label = 'obstacle points', marker = '+', c = 'black', alpha = 1)  # 固定障害物点
    ...
    def animate_non_move_obs(i):
        ...
        return center, cpoints[0], cpoints[1], cpoints[2], cpoints[3], cpoints[4], cpoints[5], cpoints[6], cpoints[7], cpoints[8], cpoints[9], cpoints[10], cpoints[11], \
            fs[0], fs[1], fs[2], fs[3], fs[4], fs[5], fs[6], fs[7], fs[8], fs[9], fs[10], fs[11], time_text, \
